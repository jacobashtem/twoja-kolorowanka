...
- [x] **Konto i aplikacja API założone, token OAuth działa (2026-09-17).** Domena zweryfikowana tagiem w `nuxt.config.ts`. Sekrety w GitHub Actions wgrane.
- [ ] **BLOKADA: aplikacja ma dostęp Trial, nie Standard.** Trial pozwala czytać i tworzyć boardy, ale `POST /pins` na produkcji zwraca 403 (kod 29). Pierwsza próba 2026-09-17: 4 boardy powstały, 0 pinów. Wniosek o Standard access składa się w panelu aplikacji na developers.pinterest.com; wymaga nagrania wideo z działania aplikacji (przebieg OAuth i utworzenie pina, może być w sandboxie `api-sandbox.pinterest.com`). Do czasu zgody cron `pinterest-daily` będzie codziennie kończył się błędem 403 bez skutków ubocznych.
...
- [x] Boardy: **board per kategoria** (decyzja Jakuba, 2026-09-17), 12 wpisów w `data/pinterest-boards.json`. Każda kolejna podmieniona kategoria dostaje własny wpis; bez wpisu wpada do boardu tematycznego po pierwszym segmencie ścieżki.
